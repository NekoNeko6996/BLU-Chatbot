...
async def upload_file(folder: str = Form(...), file: UploadFile = File(...)):
    ...
    try:
        ...
        def process_and_replace_image(match):
            ...
            # Gọi API lấy mô tả ảnh
            caption = analyze_image_with_gemini(b64_data, ext)
            
            # Chèn mô tả vào ngay dưới mã ảnh
        ...
        raise HTTPException(status_code=500, detail=f"Lỗi convert file: {str(e)}")

    log_action("Upload", file.filename, folder)
    return {"message": "Upload và xử lý thành công"}
...
async def save_md_content(data: MDContent):
    ...
    log_action("Sửa file MD", data.filename, data.folder)
    return {"message": "Lưu file thành công"}
...
async def delete_file(folder: str, filename: str):
    ...
    # 2. Xóa MD
    md_path = target_doc_dir / f"{slug_name}.md"
    if md_path.exists():
        md_path.unlink()
        deleted = True
        
    # 3. Xóa gốc (Quét theo stem để bắt chính xác bản pdf/docx tương ứng)
    if target_raw_dir.exists():
        for raw_file in target_raw_dir.iterdir():
            if raw_file.is_file() and slugify(raw_file.stem) == slug_name:
                raw_file.unlink()
                deleted = True

    # 4. Xóa bản cũ (.txt)
    if target_doc_dir.exists():
        for doc_file in target_doc_dir.iterdir():
            if doc_file.is_file() and doc_file.suffix.lower() != '.md' and slugify(doc_file.stem) == slug_name:
                doc_file.unlink()
                deleted = True
    
    # 5. Xóa dữ liệu vector trong DB    
    try:
        client = QdrantClient(path="database/collection")
        client.delete(
            collection_name="blu",
            points_selector=models.Filter(
                must=[models.FieldCondition(key="metadata.source", match=models.MatchValue(value=f"{slug_name}.md"))]
            ),
        )
    except Exception as e:
        print(f"Lỗi xóa vector cho file {slug_name}: {e}")
        
    if not deleted: raise HTTPException(status_code=404, detail="Không tìm thấy file để xóa")
    log_action("Xóa file", filename, folder)
    return {"message": "Đã xóa file và các tài nguyên liên quan thành công"}
...
async def create_backup(payload: BackupRequest):
    if not DB_DIR.exists() or not any(DB_DIR.iterdir()):
        raise HTTPException(status_code=400, detail="Database hiện tại đang trống, không có dữ liệu để sao lưu.")
    
    backup_path = BACKUP_DIR / payload.name
    if backup_path.exists():
        raise HTTPException(status_code=400, detail="Tên bản sao lưu đã tồn tại.")
    ...
    try:
        ...
        log_action("Sao lưu DB", payload.name, "Backup")
        return {"message": "Sao lưu thành công"}
    except Exception as e:
    ...
    if BACKUP_DIR.exists():
        for item in BACKUP_DIR.iterdir():
            if item.is_dir():
                ...
                old_meta = item / "meta.json"
                
                # Load bản backup chuẩn
                if info_file.exists():
                    import json
                    with open(info_file, "r", encoding="utf-8") as f:
                        meta = json.load(f)
                        backups.append(meta)
                # Load bản backup cũ (bị lỗi) để hiển thị lên UI cho người dùng xóa
                    ...
                    with open(old_meta, "r", encoding="utf-8") as f:
                        try:
                            meta = json.load(f)
                            if "name" in meta:
                                meta["description"] = "[FILE LỖI - HÃY XÓA BẢN NÀY] " + meta.get("description", "")
                                backups.append(meta)
                        except: pass
...
async def restore_backup(name: str):
    ...
    # Chặn không cho khôi phục các bản backup cũ bị lỗi
    if (backup_path / "meta.json").exists() and not (backup_path / "backup_info.json").exists():
        raise HTTPException(status_code=400, detail="Bản sao lưu này đã bị hỏng cấu trúc do lỗi hệ thống cũ. Vui lòng xóa bản sao lưu này đi!")

    try:
        if DB_DIR.exists():
            shutil.rmtree(DB_DIR)
        shutil.copytree(backup_path, DB_DIR)
        
        # Xóa file thông tin rác nếu bị copy nhầm vào db hoạt động
        if (DB_DIR / "backup_info.json").exists():
            (DB_DIR / "backup_info.json").unlink()
            
        log_action("Khôi phục DB", name, "Backup")
        return {"message": "Khôi phục thành công. Vui lòng khởi động lại Chatbot Server."}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
